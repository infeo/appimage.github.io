...
if [ x"${URL:0:18}" == x"https://github.com" ] && [[ "${URL}" != *"download"* ]] ; then # do not redirect direct links
  ...
  GHURL="https://api.github.com/repos/$GHUSER/$GHREPO/releases?access_token=" # Not "/latest" due to https://github.com/AppImage/AppImageHub/issues/12
...
if [ x"${URL:0:22}" == x"https://api.github.com" ] || [ x"${GHURL:0:22}" == x"https://api.github.com" ] ; then
  ...
  echo "GitHub API URL detected: $GHURL"
  ...
  if [ x"" == x"$URL" ] ; then
    echo "No 64-bit one found, trying any"
